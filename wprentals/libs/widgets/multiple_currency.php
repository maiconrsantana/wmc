	...
	function widget($args, $instance){
		...
		if($title) {
			print $before_title.$title.$after_title;
                ...
                $cur_list.='<li  role="presentation" data-curpos="'.$where_currency.'"  data-coef="1" data-value="'.$normal_cur.'" data-symbol="'.$normal_label.'" data-pos="-1">'.$normal_cur .' </li>';
                ...
                if( !empty($multiple_cur)){  
                    while($i< count($multiple_cur) ){
                        $cur_list.='<li  role="presentation" data-curpos="'.$multiple_cur[$i][3].'" data-coef="'.$multiple_cur[$i][2].'" data-value="'.$multiple_cur[$i][0].'" data-symbol="'.$multiple_cur[$i][0].'" data-pos="'.$i.'"> '. $multiple_cur[$i][0].' </li>';
                        $i++;
                    }
                }$normal_label   =    get_option('wp_estate_currency_label_main','');
