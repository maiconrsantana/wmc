...
?>
 ...
 <div class="adv-2-header">
     <?php esc_html_e('Faça sua Reserva','wpestate');?>
</div>  
...
<div class="adv-search-2 <?php echo $google_map_lower_class.' '.$close_class.' '.$extended_class;?>" id="adv-search-1" data-postid="<?php echo $post_id; ?>"> 
    ...
    <form  method="get"  id="main_search" action="<?php print $adv_submit; ?>" >
                ...
                <div data-toggle="dropdown" id="guest_no" class="filter_menu_trigger"  
                    ...
                    if(isset($_GET['guest_no'])){
                       echo intval($_GET['guest_no']).' '.esc_html__('guests','wpestate');
                        ...
                        esc_html_e('Convidados','wpestate');
                    }?>  
        ...
        <div class="col-md-12">
        <input name="submit" type="submit" class="wpb_btn-info wpb_btn-small wpestate_vc_button  vc_button" id="advanced_submit_2" value="<?php esc_html_e('Pesquisar','wpestate');?>">
        </div>
              
        <div id="results">
            <?php esc_html_e('Nós encontramos ','wpestate')?> <span id="results_no">0</span> <?php esc_html_e('resultados.','wpestate'); ?>  
            <span id="showinpage"> <?php esc_html_e('Mostrar os resultados agora ?','wpestate');?> </span>
        </div>
